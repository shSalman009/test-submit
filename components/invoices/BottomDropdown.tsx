...
interface Props {
  data: MyObject[];
  handleClick?: (e: string, id: number) => void;
  id: number;
  value: string;
}
...
export default function BottomDropdown({
  ...
  // Search Term for search dropdown item
  ...
  // Select Item
  const handleSelect = (item: MyObject) => {
    if (handleClick) {
  ...
  return (
    <>
      <div
        ...
        className="cursor-pointer relative text-sm border text-center inline-flex items-center border-gray-300 outline-none px-4 py-3 text-gray-900 w-full rounded-lg"
      >
        <input
          type="text"
